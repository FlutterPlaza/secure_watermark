...
void main(List<String> arguments) {
  ...
  try {
    ...
    stderr.writeln();
    stderr.writeln('Usage: dart run secure_watermark:extract [options]');
    stderr.writeln(parser.usage);
    exit(64); // EX_USAGE
  }

  if (args.flag('help')) {
    stdout.writeln('Extract a forensic watermark from a PNG image.');
    stdout.writeln();
    stdout.writeln('Usage: dart run secure_watermark:extract [options]');
  ...
  if (payload != null) {
    ...
    stderr.writeln('No watermark found (wrong key or no watermark present)');
    exit(1);
  }
}
